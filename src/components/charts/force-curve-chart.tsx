...
import { CHART_DASHES as DASHES, CHART_HEIGHT as HEIGHT, CHART_PADDING as PAD, CHART_WIDTH as WIDTH, dashLabel, formatDate, scale } from "./chart-utils";
  ...
  return (
    <figure aria-labelledby="force-chart-title" style={{ margin: 0 }}>
      ...
      <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-describedby="force-chart-description" style={{ width: "100%", height: "auto", marginTop: 16 }}>
        <desc id="force-chart-description">Force against elapsed time. Series use different line patterns and all samples are available in tables below.</desc>
      ...
      <details>
        <summary>Show equivalent force samples</summary>
        {visible.map((curve) => {
          const entry = entryByAttempt.get(curve.attemptId)!;
