...
export default async function GroupPage({ params, searchParams }: { params: Promise<{ groupId: string }>; searchParams: Promise<{ message?: string }> }) {
  ...
  return <AppShell profileName={profile.display_name} groups={groups} activeGroupId={groupId}>
    <header className="page-header"><p className="eyebrow">{role}</p><h1>{detail?.name ?? "Group"}</h1><p>Members share immutable training protocols and comparable results.</p><div className="button-row"><Link className="button" href={`/dashboard?group=${groupId}`}>Open dashboard</Link><Link className="button button-quiet" href={`/groups/${groupId}/protocols`}>Protocols</Link>{canManage && <><Link className="button button-quiet" href={`/groups/${groupId}/moderation`}>Moderation</Link><Link className="button button-quiet" href={`/groups/${groupId}/audit`}>Audit</Link></>}</div></header>
